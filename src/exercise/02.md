...
Memoization is a performance optimization technique which eliminate the need to
...
Another aspect of memoization is value referential equality.

If you've got two objects, two functions, or two arrays, even if they are word
for word defined the same, if you created them separately, they are not equal.
That is because two objects, functions, or arrays cannot be the same. Ex:
...
const func1 = param => console.log(param)
const func2 = param => console.log(param)
console.log(func1 === func2) // false
```

For objects, arrays, and functions, the `===` operator checks if they reference
the same location in memory. Even if two objects, arrays, or functions have the
same structure or content, they are considered different because they occupy
different locations in memory.

Although `===` will never return true for reference types like object, array,
and function, you can use memoization to get the same reference. Ex:

```typescript
function memoize<ArgType, ReturnValue>(cb: (arg: ArgType) => ReturnValue) {
  const cache = new Map<ArgType, ReturnValue>()
  return function memoized(arg: ArgType) {
    if (cache.get(arg) === undefined) {
      cache.set(cb(arg))
    }
    // you'll get eslint error that memoized shouldn't return `undefined` but you
    // can use an eslint ignore tag because if it was undefined we'd have just
    // set it above
...
const getDog = memoize((name: string) => new Dog())
const dog1 = getDog('fido') // creates dog fido cuz not in map
const dog2 = getDog('fido') // returns same dog object because it exists in the map
console.log(dog1 === dog2) // true because `getDog` returned the same object
```

### Memoization in React

React has its own memoization abstractions, `useMemo` and `useCallback`.

`React.useEffect` can be used when primitives are being used, like numbers or
strings.

When reference types like objects, arrays, or functions get declared in the
scope of the `useEffect` and are dependencies of the hook, those reference types
get re-created every time the component renders, which causes an endless render
cycle.

When a function gets used in a `React.useEffect`, what do you do provide in the
dependency array in this scenario? You'd have to pass the function to the
dependency array, but as mentioned above, that'll get re-created every render.

One solution is to maybe include any dependencies of the function used in
`useEffect` in the dependency array, but then you'd have to make sure that if
you update said function and change its dependencies, you'd have to remember to
update the dependency list of the `useEffect` as well.
...
const updateLocalStorage = React.useCallback(
  ...
  [count], // <-- There it is! A dependency list!
)
```

What this does is, when ever this re-renders, it **will** initialize the
function every time, but if nothing in the dependency array changed, React will
give us back the same function.

`useCallback` is just a shortcut for using `useMemo` for functions.

if you were to use `useMemo` you'd have to wrap the callback in anonymous
function causing a double-arrow function sort of thing:
...
dependency lists and props on memoized component. The only time it's useful to
use `useCallback` is when the function you're memoizing is used in of those two
situations
...
**NOTICE: Things have changed.** React 18 has since
[deprecated the warning](https://github.com/facebook/react/pull/22114)
highlighted in this exercise. This extra credit has been left here in-case you
are **curious only**. You **should not** expect to be able to reproduce the
scenario as described in the exercise.
