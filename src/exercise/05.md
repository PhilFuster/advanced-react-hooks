...
The React `useImperativeHandle` hook allows for exposing imperative APIs for
consumers of a component to use by attaching these methods to a passed `ref`.

Let's say I have a component that displays messages between users. It's a
scrollable container, that allows for scrolling as messages fill the container.

If this display component wanted to expose a way for consumers to scroll to the
top or bottom, it'd be quite hard, because there'd be no way to pass the
reference to the container out from the component.

`useImperativeHandle` allows for the consumer of this display component to
provide a `ref` and the display component will attach methods that allow for
consumers to scroll to top and scroll to bottom of the message container.
