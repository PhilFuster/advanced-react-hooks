...
through components that don't make use of it, just so a nested component further
down the tree can access that data.

One solution to this issue is React's Context feature.

By creating context using the `React.createContext` hook, you can then wrap any
...
function App() {
  return (
    <UserContextProvider>
      {/* All components wrapped in `UserContextProvider` can get access to the context*/}
      <main>
...
Many believe that React Context is overkill in most scenarios. Context should be
used for truly global state. Instead of using React Context, first try to opt
for better composition of components.
...
opposed to being a black box that controls all markup that gets rendered.
...
Then `App` could instead pass the children that are supposed to get rendered in
the `Profile` and provide the `username` state to the component that needs it,
`ProfileBody`, avoiding prop drilling and having to use React Context.
...
Instead of calling `React.useContext` and having to import the context
everywhere, you can encapsulate the logic accesses the context. This consumer
hook is responsible for throwing an error if the context cannot be accessed.
