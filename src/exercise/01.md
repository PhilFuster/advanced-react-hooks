...
### Simulating `this.setState` from class components
...
function as the action; the reducer just has to be able to handel the state
appropriately.

There are a handful of ways you can implement the use of `React.useReducer` to
help you achieve your goals as a developer. You can implement it such that:

- The consumer provides the new state already calculated. This puts the
  responsibility of updating state to the consumer. The reducer just has to
  overwrite current state with the new state passed in.
- The consumer provides the dispatch function the details and all the complexity
  of state merging is encapsulated in the reducer.
- A hybrid approach, where some values are just taken as is and others are
  derived from the new state.
- Passing in an object with an `action` property that acts as the `action` for
  the reducer to perform on the provided state.
