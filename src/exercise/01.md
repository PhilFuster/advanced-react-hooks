...
`stateReducer` gets called with the current state and whatever `setState` gets
called with.
...
For example, to manage the state of a simple counter you could:

```js
const [counter, setCounter] = react.useReducer((count, newCount) => newCount, 0)
```

the inlined `reducer`, accepts the current state and the new state. In this
case, the new state would be whatever `setCounter` gets called with. You could
imagine it'd be something like `setCounter(counter + 1)`.
