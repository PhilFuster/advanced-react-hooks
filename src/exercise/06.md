...
`React.useDebugValue` is only useful in custom hooks. It allows you to provide a
label that'll show up in the React Dev Tools when you use multiples of the same
hook. Without `useDebugValue` you'll have a harder time telling which hook usage
is which definition.

#### `useDebugValue`'s second argument

You can also pass `useDebugValue` a formatter function. This is only useful if
the debug value is an expensive computation you only want to run when the React
Dev Tools are visible.
